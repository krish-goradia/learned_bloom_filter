...
| Target FPR | n_features | Threshold | Backup FPR | System FPR | System FNR | Learned Memory (MB) | Standard Memory (MB) | Memory Reduction |
|---|---|---|---|---|---|---|---|---|
| 0.1 | 1024 | 0.4 | 0.05 | 0.082 | 0.263 | 0.0206 | 0.0302 | **31.8%** |
| 0.05 | 1024 | 0.7 | 0.05 | 0.048 | 0.416 | 0.0279 | 0.0393 | **29.0%** |
| 0.01 | 1024 | 0.9 | 0.01 | 0.0086 | 0.596 | 0.0510 | 0.0605 | **15.7%** |
| 0.005 | 4096 | 0.8 | 0.001 | 0.0044 | 0.384 | 0.0721 | 0.0696 | **-3.6%** |
...
This quantifies a clear operating regime boundary: **the learned BF only wins on memory when FPR ≥ 0.01**.

### 5.2 Latency

| Component | Avg Latency (ns/query) |
|---|---|
| Model alone | ~90–94 |
| BF alone | ~105–106 |
| Total system | ~178–194 |
| Standard BF | ~108–110 |
...
The backup BF provides meaningful coverage only when the same bad URLs appear in both train and test sets. A random 70/30 split ensures **disjoint** URL sets, so the backup BF provides near-zero practical benefit in this evaluation.

In a realistic deployment (e.g. a web proxy querying a known-malicious URL repeatedly), there would be significant overlap between historical and live bad URLs, making the backup BF genuinely useful.
...
### 6.4 Memory Accounting Asymmetry

The standard BF memory figure includes only BF bits. The learned BF memory includes both model weights and BF bits. This is the correct total cost comparison, but the standard BF has no model overhead by definition — the comparison is inherently asymmetric and should be read as "total system memory" not "apples-to-apples BF comparison."
